...
IO::RVIZHelper::RVIZHelper(const RobotConstPtr &robot, const std::string &name)
  ...
  std::string semantic;
  robot_->getHandlerConst().getParam(
      Robot::ROBOT_DESCRIPTION + Robot::ROBOT_SEMANTIC, semantic);

  nh_.setParam(Robot::ROBOT_DESCRIPTION, description);
  nh_.setParam(Robot::ROBOT_DESCRIPTION + Robot::ROBOT_SEMANTIC, semantic);

  trajectory_pub_ =
      nh_.advertise<moveit_msgs::DisplayTrajectory>("trajectory", 1);
...
void IO::RVIZHelper::updateTrajectory(
  ...
  moveit::core::robotStateToRobotStateMsg(
      response.trajectory_->getFirstWayPoint(), out.trajectory_start);

  trajectory_pub_.publish(out);
}

void IO::RVIZHelper::updateTrajectories(
    const std::vector<planning_interface::MotionPlanResponse> &responses) {
  moveit_msgs::DisplayTrajectory out;
  out.model_id = robot_->getModelName();
...
void IO::RVIZHelper::addTextMarker(const std::string &name,
                                   ...
                                   const std::string &base_frame,
                                   const RobotPose &pose, double height,
                                   const Eigen::Vector4d &color) {
  visualization_msgs::Marker marker;
...
void IO::RVIZHelper::addGoalMarker(const std::string &name,
  ...
  const auto &base_frame =
      "map"; // Markers must be placed in "map" (default RViz frame)

  // Iterate over each goal (an "or"-ing together of different constraints)
  for (const auto &goal : goals) {
    auto color =
        getRandomColor(); // Use the same color for all elements of this goal
    ...
    for (const auto &pg : goal.position_constraints) {
      ...
      for (const auto &primitive :
           boost::combine(pg.constraint_region.primitives,
                          pg.constraint_region.primitive_poses)) {
        ...
        addGeometryMarker(name, Geometry::makeSolidPrimitive(solid), base_frame,
                          frame, color);
        ...
        for (const auto &og : goal.orientation_constraints) {
          ...
          Eigen::Vector3d scale = {0.1, 0.008,
                                   0.003}; // A nice default size of arrow
...
void IO::RVIZHelper::removeScene() { updateScene(nullptr); }
