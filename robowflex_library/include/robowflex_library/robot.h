...
namespace robowflex
{
     ...
     *
     *  The Robot class is a wrapper around _MoveIt!_'s robot_model::RobotModel and a "scratch" robot_state::RobotState.
     *  Mostly, this class handles loading the relevant information to the parameter server without the use of a launch
     *  file. The necessary files are the URDF and SRDF (both either XML or xacro files), and the joint limits and
     *  kinematics plugin (both YAML files). These pieces of information are loaded to the parameter server under the
     *  name provided to the constructor so that multiple robots can be used. Note that all information is also loaded
     *  under the unique namespace generated by the IO::Handler.
     *
     *  By default, no IK kinematics plugins are loaded due to startup costs (particularly for R2). You must call
     *  loadKinematics() with the desired planning groups (as defined by the SRDF) if you want IK.
     *
     *  Additionally, post-processing function hooks are provided if modifications need to be done to any of the
     *  necessary files. This enables the addition of extra joints, semantic information, joint limit overloading, etc.
     *  as needed. See FetchRobot for an example use of adding a new virtual joint to the SRDF.
     */
    class Robot
    {
    public:
