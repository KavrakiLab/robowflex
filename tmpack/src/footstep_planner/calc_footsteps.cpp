...
double eval_step(point_2D p1, point_2D p2) {
  return (FLAGS_walk_centroid - dist(p1, p2)) *
         ...
         (dist(p1, p2) > FLAGS_step_max_dist ? 1000000000 : 0);
}
...
std::vector<point_2D> getFootPlacementsFromTorsoPose(std::vector<point_2D> points,
  ...
  std::cout << "number of candidate points: " << tmp_points.size()
            << std::endl;
...
class FootstepPlanner {
 ...
 public:
  ...
  std::vector<point_2D> calculateFootPlacementsForTorso(
      std::vector<point_2D> points, point_2D start, point_2D torso_goal,
      foot start_foot) {

    std::vector<point_2D> final_feet_placements = getFootPlacementsFromTorsoPose(points, torso_goal);

    std::vector<point_2D> seq1 = calculateFootPlacements(points, start, final_feet_placements[0], start_foot);
    std::vector<point_2D> seq2 = calculateFootPlacements(points, start, final_feet_placements[1], start_foot);
