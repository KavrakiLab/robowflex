...
def add_planning_scene_world(name, world):
    '''Adds a moveit_msgs::PlanningSceneWorld to the blender scene.
    '''
    # TODO: add Octomap
    add_collision_objects(name, world['collision_objects'])


def add_planning_scene(name, scenefile):
    '''Reads in a scene yaml (output by robowflex_library) file and adds all
    collision objects to the blender scene.
    '''
    scene = rv.utils.read_YAML_data(scenefile)
    add_planning_scene_world(name, scene['world'])
