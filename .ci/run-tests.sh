...
function check-docker-tests()
{
    ...
    docker exec ${container}_test /bin/bash -c "source ros_entrypoint.sh; cd /ws/; catkin run_tests robowflex_moveit --no-deps --limit-status-rate 0.001 --no-notify; catkin_test_results"
    local result=$?
