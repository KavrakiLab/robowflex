...
namespace robowflex
{
    ...
    public:
        ...
        struct Options
        {
            ...
            double improve_ratio_threshold{0.25};  ///< Minimum ratio true_improve/approx_improve to accept step
            double min_trust_box_size{1e-4};       ///< If trust region gets any smaller, exit and report convergence
            double min_approx_improve{1e-4};       ///< If model improves less than this, exit and report convergence
            double min_approx_improve_frac{-std::numeric_limits<double>::infinity()};  ///< If model improves less than this, exit and report convergence
            double max_iter{100.0};                 ///< The max number of iterations 
            double trust_shrink_ratio{0.1};       // If improvement is less than improve_ratio_threshold, shrink trust region by this ratio
            double trust_expand_ratio{1.5};  ///< If improvement is less than improve_ratio_threshold, shrink trust region by this ratio
            double cnt_tolerance{1e-4};       ///< After convergence of penalty subproblem, if constraint violation is less than this, we're done
         ...
         */
        bool plan(const SceneConstPtr &scene, const robot_state::RobotStatePtr &start_state, const robot_state::RobotStatePtr &goal_state);
         ...
         *  \return True if a plan was successfully computed.
         */
        bool plan(const SceneConstPtr &scene, const robot_state::RobotStatePtr &start_state, const Eigen::Isometry3d &goal_pose, const std::string &link);
         ...
         *  \return True if a plan was successfully computed.
         */
        bool plan(const SceneConstPtr &scene, const std::unordered_map<std::string, double> &start_state, const Eigen::Isometry3d &goal_pose, const std::string &link);
    ...
    private:
         ...
         */
        void setOptimizerParameters(std::shared_ptr<tesseract::tesseract_planning::TrajOptPlannerConfig> &config) const;
