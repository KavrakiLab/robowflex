...
namespace robowflex
{
    ...
    public:
         ...
         */
        void setInitialTrajectory(const trajopt::TrajArray &init_trajectory){initial_trajectory_ = init_trajectory;
         ...
         */
        const std::vector<std::string> getEnvironmentLinks(){return env_->getLinkNames();};
        
        /** \brief Gets the link names of the env manipulator.
         */
        const std::vector<std::string> getManipulatorLinks(){return env_->getManipulator(manip_)->getLinkNames();};
        
        /** \brief Gets the joint names of the env manipulator.
         */
        const std::vector<std::string> getManipulatorJoints(){return env_->getManipulator(manip_)->getJointNames();};
        ...
        /** \brief Plans a motion given a \a start_state, a cartesian \a goal_pose for a \a link and a \a scene.
         *  \param[in] scene A planning scene for the same \a robot_ to compute the plan in.
         *  \param[in] start_state Start state for the \a robot_.
    ...
    private:
        ...
        /** \brief Loads a new scene into the KDL environment.
        ...
        /** \brief Transforms a \a robot_state to a vector representing joint values for the manipulator (in the correct order).
         *  \param[in] robot_state Robot state to be transformed.
         *  \param[out] manip_state Manipulator's joint values representing \a robot_state.
         */
        void roboStateToManipState(const robot_state::RobotStatePtr &robot_state, 
                                   std::vector<double> &manip_state);
        
        /** \brief Transforms a tesseract \a waypoint to robot \a state. 
         *  Joint values that are not in the manipulator are taken from the tesseract environment.
